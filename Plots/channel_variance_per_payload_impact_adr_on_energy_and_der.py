...
plt.subplots_adjust(wspace=1.2, hspace=2.2)
...
for ax_id, air_f, gateway_f, node_f in zip(ax_id, air_interface_files, gateway_files, node_files):
    ...
    for p in payload_sizes:
        ...
        for var, en, rx, tx in zip(path_loss_variances, node_df.mean_energy_per_byte, node_df.UniquePackets,
                                   gateway_df.UniquePacketsReceived):
            channel_variance[var][p] = ((rx / tx) * 100) / en
