...
def plot_time(_env):
    while True:
        ...
        yield _env.timeout(np.round(simulation_time / 100))
...
def make_pickles(adr, confirmed_messages):
    ...
    directory = f'./Measurements/ChannelVariance/paper_check/{adr_prefix}_{conf_prefix}_'
...
make_pickles(adr=True, confirmed_messages=True)
print("-----------MAKE 1 IS DONE----------")
make_pickles(adr=True, confirmed_messages=False)
print("-----------MAKE 2 IS DONE----------")
make_pickles(adr=False, confirmed_messages=False)
