...
def run_simulation(conf, adr, training, num_nodes):
    ...
    for node_id in range(num_nodes):
        ...
        node = Node(node_id, energy_profile, lora_param, sleep_time=(8 * payload_size / transmission_rate),
                    ...
                    training=training,
                    base_station=gateway, env=env, payload_size=payload_size, air_interface=air_interface, reward_type="normal")
...
def init_nodes(conf, adr, training, num_nodes, reward_function, locations, state_space, replay_buffer=False, agent_to_nodes=None, sector_size=100, deep=False, sarsa=False):
    energy_per_bit = 0
    tx_power_mW = {2: 91.8, 5: 95.9, 8: 101.6, 11: 120.8, 14: 146.5}  # measured TX power for each possible TP
    middle = np.round(Config.CELL_SIZE / 2)
    gateway_location = Location(x=middle, y=middle, indoor=False)
    ...
    if (agent_to_nodes == None):
        ...
        for (cluster_center_location, cluster) in clusters.items():
            agent = DeepLearningAgent(env=env, depth=4, state_space_dimensions=len(state_space), gamma=0.5, epsilon=0.9, lr=0.001, sarsa=sarsa, replay_buffer=replay_buffer) if deep \
...
def compare_before_and_after(title="normal"):
    ...
    # "state_space": ["tp", "sf", "channel", "sinr", "rss", "energy", "packet_id", "num_pkt"]},
    configurations = [
        {
            ...
            "days": 10,
            "locations": locations,
            "state_space": ["tp", "sf", "channel", "sinr"]},
        {
            "title": "Q learning",
            "conf": False,
            "adr": False,
    ...
    for config_cnt, config in enumerate(configurations):
        ...
        if first_run:
            dimensions = 7.5 * len(configurations)
            f, axarr = plt.subplots((num_categories * 2) + 2, len(configurations), figsize=(dimensions, dimensions), sharex='col', sharey='row')
        first_run = False
...
def compare_t_b_and_a(title="new"):
    ...
    nodes_before, env_before = init_nodes(conf=False, adr=False, training=False,
                                          num_nodes=num_nodes, reward_function=title, locations=locations)
...
def compare_thrughput_before_and_after_for_all_nodes(title="throughput"):
    ...
    nodes_before, env_before = init_nodes(conf=False, adr=False, training=False, num_nodes=1000, reward_function=title, locations=locations)
    nodes_adr_conf, env_adr_conf = init_nodes(conf=True, adr=True, training=True, num_nodes=1000, reward_function=title, locations=locations)
    nodes_after, env_after = init_nodes(conf=False, adr=False, training=True, num_nodes=1000, reward_function=title, locations=locations)
...
def compare_before_and_after(configurations):
...
compare_before_and_after(configurations=configurations)
